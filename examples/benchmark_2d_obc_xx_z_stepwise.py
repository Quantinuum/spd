...
def build_one_step_circuit(n, h, dt):
    """Build one Trotter step matching the legacy benchmark convention.

    SPD propagates observables in the Heisenberg picture, so operations are
    applied in reverse circuit order. The legacy benchmark evolves with the
    XX terms followed by the Z terms; therefore the pytket circuit is built as
    Z followed by XX.
    """
...
def evolve_step(spo, operations, adapter, trunc_val, max_num_str):
    ...
    if adapter.name == "triton":
        from spd.triton_backend import evolve_step as triton_step
        warm = triton_step(spo, operations, trunc_val, max_num_str)
        warm.synchronize()
        del warm
        t0 = time.perf_counter()
        next_spo = triton_step(spo, operations, trunc_val, max_num_str)
        next_spo.synchronize()
        return next_spo, time.perf_counter() - t0
    ...
    next_spo.c_array.block_until_ready()
    return next_spo, time.perf_counter() - t0
...
def main():
    ...
    parser.add_argument("--backend", choices=["numpy", "jax", "triton"], default="jax")
    ...
    if args.backend == "triton":
        ...
        adapter = SimpleNamespace(name="triton")
    else:
    ...
    if args.backend == "triton":
        observable = create_op(build_initial_observable(args.n),
                               padded_system_size(num_qubits), args.precision)
    else:
        observable = adapter.create_initial_spo(
            build_initial_observable(args.n), padded_system_size(num_qubits),
        )
    ...
    if args.backend == "jax":
        ...
        print("Triton timing mode: warm one pass per step; synchronized second pass")
    print(

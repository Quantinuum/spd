...
if __name__ == "__main__":
    ...
    for backend_name in ['numpy', 'jax']:
        for basis in ['Z', '+']:
            ...
            combine_grads = np.array(combine_grads)
            ...
            assert np.allclose(exp_val, exact_exp_val), f"Expectation value does not match for basis {basis} and backend {backend_name}"
            assert np.allclose(combine_grads, exact_grads, rtol=1e-5), f"Gradients do not match for basis {basis} and backend {backend_name}"
            ...
            # for eps in [1e-2, 1e-5]:
                ...
                for i in range(number_of_parameters):
                    ...
                    if run_dim == 1:
                        ...
                        new_circ = tfi_setup.gen_2d_TFI_ansatz_circuit(new_random_thetas,
                                                                       system_size_x,
                                                                       system_size_y,
                                                                       )
                        ham_dict = tfi_setup.gen_2d_Hamiltonian_dict(system_size_x, system_size_y, g=3.1)

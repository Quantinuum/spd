...
p=argparse.ArgumentParser();p.add_argument('--backend',choices=['mono','cu','persistent','anti','main'],required=True);p.add_argument('--prefix',type=int,default=0);a=p.parse_args()
...
nq=216;cap=20000000;cutoff=1e-5
out=f'/tmp/afh6-sanity/{a.backend}.json';result=dict(backend=a.backend,qubits=nq,padded_qubits=224,rotations=len(rots),preparation_gates=len(cliffs),cutoff=cutoff,cap=None if a.backend=='mono' else cap,threads=12,thetas=theta.tolist(),circuit_sha256=hashlib.sha256(repr(ops).encode()).hexdigest(),blocks=[],status='running',timing='one pass; small synthetic Triton compilation preflight excluded; block synchronization included; observations/logging excluded')
...
if a.backend=='mono':
 ...
 def apply(i):
  ...
  if state.num_terms>cap:
   c=state.coeffs[:state.num_terms];t=torch.from_dlpack(c);idx=torch.topk(t.abs(),cap,sorted=False).indices;ci=cp.from_dlpack(idx)
   nk=state.xz_bits.reshape(-1,2*w)[:state.num_terms][ci];nc=c[ci]
   state=PauliExpansion(handle,nq,cap,nk,nc,has_duplicates=False,options=options)
 ...
 result['cap_method']='external torch.topk and CuPy gather/reconstruct, included in time'
else:
 import torch
...
try:
 for i in range(len(rots)):
  ...
  if n>=cap and first_cap is None:first_cap=i+1
  stopped=a.backend=='mono' and n>cap
  if (i+1)%216==0 or i+1==a.prefix or i+1==len(rots) or stopped:
   ...
   result['blocks'].append(row);result['rotation_seconds']=sum(r['seconds'] for r in result['blocks']);result['completed_rotations']=i+1;result['first_cap_gate']=first_cap
   save();print(json.dumps(row),flush=True)
   if stopped:
    result['status']='stopped_at_term_boundary';break
